...
| redis_keepalive_timeout | integer           | False                                     | 10000         | ≥ 1000                     | Keepalive timeout in milliseconds for Redis when `policy` is `redis`, `redis-cluster`, or `redis-sentinel`. Defaults to `60000` when `policy` is `redis-sentinel`. |
| redis_keepalive_pool    | integer           | False                                     | 100           | ≥ 1                        | Keepalive pool size for Redis when `policy` is `redis`, `redis-cluster`, or `redis-sentinel`. |
